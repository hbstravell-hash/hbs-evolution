...
export default function PackagePage({ params }: { params: { slug: string } }) {
  ...
  const pkgHotels = hotels.filter((hh) => pkg.hotels.includes(hh.slug));
    ...
    const whatsappHref = 'https://wa.me/995500000000?text=' + whatsappMsg;
    ...
    { className: 'container-p', style: { padding: '28px 20px' } },
        ...
        h(
          ...
          { style: { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px', marginBottom: '20px' } },
                [1, 2, 3, 4].map((n) =>
                          h('div', { key: n, className: 'gallery-placeholder', style: { height: '140px', fontSize: '12px' } }, 'صورة احترافية ' + n + ' (سيتم استبدالها بصور تصويرية حقيقية)')
                                       )
              ),
        h(
                'div',
                ...
                h(
                  ...
                  { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '14px' } },
                          pkgHotels.map((hh) =>
                                      h(
                                                    Link,
                                        { key: hh.slug, href: '/hotels/' + hh.slug, className: 'card', style: { padding: '14px', textDecoration: 'none', color: 'inherit' } },
